...
List spline_udpate(const mat &Z_tv, const mat &B_spline, mat &theta, 
  ...
  NumericVector AIC_all, TIC_all, TIC2_all, GIC_all, HIC_all;
  mat VarianceMatrix;
  ...
  while (iter < iter_max) {
    ...
    if (btr=="none") {
      ...
      if (stop=="incre")
        ...
        if(iter > 1){
          mat theta_tmp2 = theta_list[iter-2];
  ...
  if(ICLastOnly == true){
    ...
    //HIC: hastie:
    df = trace(2*info_lambda_inv*info - (info_lambda_inv*info).t()*info*info_lambda_inv);
    HIC_all.push_back(-2*logplkd*N + 2*df);
...
List VarianceMatrixCalculate(const vec &event, const mat &Z_tv, const mat &B_spline,
    ...
    mat VarianceMatrix_gray = info_lambda_inv*info*info_lambda_inv;
...
// Variance Matrix Calculation
// [[Rcpp::export]]
List VarianceMatrixCalculate_bresties(const vec &event, const vec &time, 
                                      const mat &Z_tv, 
                                      const mat &B_spline,
                                      mat &theta,
                                      double lambda_i,
...
List spline_udpate_lambdafromlarge(const mat &Z_tv, const mat &B_spline, mat &theta, 
                  const mat &Z_ti, const vec &beta_ti, 
                  mat &S_matrix,
                  double &lambda_i,
                  mat &lambda_i_mat, mat &lambda_S_matrix, 
  ...
  List objfun_list, update_list;
  NumericVector logplkd_vec;
  mat theta_init_allzero = zeros<mat>(p, K);
...
// this function is for lambda from large to small
// [[Rcpp::export]]
List surtiver_fixtra_fit_penalizestop_lambdafromlarge(const vec &event, const IntegerVector &count_strata,
                             const mat &Z_tv, const mat &B_spline, const mat &theta_init,
                             const mat &Z_ti, const vec &beta_ti_init,
                             const vec &lambda_spline,
                             const mat &SmoothMatrix,
  ...
  for (int i = 0; i < n_lambda; ++i)
  {
    ...
    mat theta_ilambda;
    if(i>0){
      mat theta_ilambda  = theta_all.slice(i-1);
    }
    else{
